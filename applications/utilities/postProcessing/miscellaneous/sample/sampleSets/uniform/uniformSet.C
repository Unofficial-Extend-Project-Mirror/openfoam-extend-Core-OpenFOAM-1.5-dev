...
) const
{
    ...
    // Normalized sampling direction
    vector curSamplingDir = smallVec;
    curSamplingDir /= mag(curSamplingDir) + VSMALL;
    ...
    while(true)
    {
        ...
        {
            //- We have reached a boundary patch.
            //  In this special situation, we abort only if we detect that the dot product between
            //  the sampling direction curSamplingDir and the face normal of the hit face exceed a threshold
            //  defined by samplingBoundaryPatchTol_. This way, using a very small threshold, we allow
            /// sampling on the surface of a boundary patch. See uniformSet.H for more details.
            //  
            vector faceNormal = mesh().faceAreas()[singleParticle.face()];
            faceNormal /= mag(faceNormal) + VSMALL;

            if( debug )
            {
                Info << "trackToBoundary : hit face normal: " << faceNormal << "current sampling dir: " << curSamplingDir << endl;
                Info << " faceNormal & curSamplingDir : " << (faceNormal & curSamplingDir) << endl;
                Info << " samplingBoundaryPatchTol_   : " << samplingBoundaryPatchTol_ << endl;
            }

            if(fabs(faceNormal & curSamplingDir) > samplingBoundaryPatchTol_)
            {
...
(
    ...
    const point& end,
    const label nPoints,
    const scalar samplingBoundaryPatchTol
)
:
    ...
    end_(end),
    nPoints_(nPoints),
    samplingBoundaryPatchTol_(samplingBoundaryPatchTol)
{
    genSamples();
...
:
    ...
    samplingBoundaryPatchTol_(   
        dict.found("samplingBoundaryPatchTol") ? // Optional entry from sampleDict:
        readScalar(dict.lookup("samplingBoundaryPatchTol")) : // take value from sampleDict
        debug::tolerances("samplingBoundaryPatchTol", // take default value from controlDict
        -1.0)) // Last resort default value: no sampling on boundary patches, as it was before
{
    genSamples();
