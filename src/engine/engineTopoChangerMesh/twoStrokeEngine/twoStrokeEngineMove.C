...
bool Foam::twoStrokeEngine::update()
{
    ...
    if (attached())
    {
        ...
        {
            Info << "Topology change; executing pre-motion after sliding detach" << endl;
...
// /* Tommaso, 23/5/2008
    ...
    if (meshChanged)
    {
        ...
        {
            Info << "Topology change; executing pre-motion after dynamic layering" << endl;
    ...
    Info << "max mesh phi, sliding mesh detached = " << max(mag(phi())) << endl;
    Info << "max mesh phi, sliding mesh detached = " << min(mag(phi())) << endl;

    const fvsPatchScalarField& pistonMeshPhi = phi().boundaryField()[boundaryMesh().findPatchID("piston")];
    
    Info << "max piston mesh phi 1 = " << max(mag(pistonMeshPhi)) << endl;
...
//*/ //Tommaso, 23/5/2008

    {
        ...
        {
            Info << "Topology change; executing pre-motion after sliding attach" << endl;
            
            Info << "topoChangeMap4->preMotionPoints().size() = " << topoChangeMap4->preMotionPoints().size() << endl;
...
//            movePoints(newPoints);
            ...
            Info << "max V()-V0() after movePoints?!? = " << max(mag(V() - V0())) << endl;
/*            
...
*/
        Info << "max mesh phi, sliding mesh attached = " << max(mag(phi().internalField())) << endl;
        Info << "max mesh phi, sliding mesh attached = " << min(mag(phi().internalField())) << endl;
            
//#           include "checkMotionFluxes.H"            
