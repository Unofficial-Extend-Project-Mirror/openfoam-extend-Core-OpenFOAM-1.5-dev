...
void Foam::doublePistonTwoStroke::correctVerticalMotion()
{
}


void Foam::doublePistonTwoStroke::calcMovingMasks() const
{
    if (debug)
    {
...
const Foam::scalarField& Foam::doublePistonTwoStroke::movingPointsMask() const
{
    if(movingPointsMaskPtr_)
    {
...
void Foam::doublePistonTwoStroke::applyMovingMask()
{
}

Foam::scalar Foam::doublePistonTwoStroke::piston2Position(const scalar theta) const
{
    return
  - (
        engTime().stroke().value()/2.0
      + engTime().clearance().value()
    )
  + (
        engTime().stroke().value()*::cos(engTime().degToRad(theta))/2.0
    );
}

Foam::dimensionedScalar Foam::doublePistonTwoStroke::piston2Displacement() const
{
    return dimensionedScalar
    (
        "piston2Displacement",
        dimLength,
        piston2Position(engTime().theta() - engTime().deltaTheta()) -
        piston2Position(engTime().theta())
    );
