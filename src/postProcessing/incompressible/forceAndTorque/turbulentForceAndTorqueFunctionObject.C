...
#include "turbulentForceAndTorqueFunctionObject.H"
...
namespace Foam
{
    defineTypeNameAndDebug(turbulentForceAndTorqueFunctionObject, 0);
    ...
    of_(time_.path()/word(dict.lookup("file")))
{
    ...
    Info<< "Creating turbulentForceAndTorqueFunctionObject for patches "
        << patchNames_ << endl;
}
...
bool Foam::turbulentForceAndTorqueFunctionObject::start()
{
    return true;
}


bool Foam::turbulentForceAndTorqueFunctionObject::execute()
{
    const fvMesh& mesh =
    ...
    forAll (patchNames_, patchI)
    {
        ...
        else
        {
            FatalErrorIn
            (
                "bool turbulentForceAndTorqueFunctionObject::execute()"
...
bool Foam::turbulentForceAndTorqueFunctionObject::read(const dictionary& dict)
{
