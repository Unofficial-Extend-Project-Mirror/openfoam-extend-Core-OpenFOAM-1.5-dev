...
)
{
    ...
    forAll(nSendCells[Pstream::myProcNo()], recvProc)
    {
        ...
        )
        {
            ...
            subCellMap[recvProc] = meshSubset.cellMap();
            subFaceMap[recvProc] = renumber
            (
                repatchFaceMap,
                meshSubset.faceMap()
            );
            subPointMap[recvProc] = meshSubset.pointMap();
            subPatchMap[recvProc] = meshSubset.patchMap();
            ...
            (
                meshSubset.subMesh(),
                meshSubset.faceMap(),        // from subMesh to mesh
                meshSubset.cellMap(),        //      ,,      ,,
            ...
            sendFields<volScalarField>(recvProc, volScalars, meshSubset);
            sendFields<volVectorField>(recvProc, volVectors, meshSubset);
            ...
            sendFields<volTensorField>(recvProc, volTensors, meshSubset);

            sendFields<surfaceScalarField>(recvProc, surfScalars, meshSubset);
            sendFields<surfaceVectorField>(recvProc, surfVectors, meshSubset);
            ...
            sendFields<surfaceTensorField>(recvProc, surfTensors, meshSubset);
        }
    }
