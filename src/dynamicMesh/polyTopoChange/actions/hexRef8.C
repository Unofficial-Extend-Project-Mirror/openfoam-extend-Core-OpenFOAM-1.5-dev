...
// This code is not compatible with topological changes being executed in one
// pass rather than dynamically.  This is due to the access of mesh points
// beofre the topo change has been performed.
// Consider re-activating if the direct topo change proves useful
// HJ, 1/Set/2007
        ...
        if (faceMidPoint[faceI] >= 0 && affectedFace.get(faceI) == 1)
        {
            ...
            forAll(f, fp)
            {
                ...
                if (pointLevel_[pointI] <= anchorLevel)
                {
                    ...
                    // This code is not compatible with topological
                    // changes being executed in one pass rather than
                    // dynamically.  This is due to the access of mesh
                    // points beofre the topo change has been
                    // performed.  Consider re-activating if the
                    // direct topo change proves useful
                    // HJ, 1/Set/2007
//                     if (debug)
//                     {
//                         if (mesh_.isInternalFace(faceI))
