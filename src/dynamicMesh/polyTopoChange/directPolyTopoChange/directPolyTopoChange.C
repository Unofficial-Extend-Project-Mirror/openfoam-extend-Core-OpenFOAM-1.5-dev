...
void Foam::directPolyTopoChange::renumberReverseMap
(
    const labelList& map,
    DynamicList<label>& elems
)
...
void Foam::directPolyTopoChange::countMap
(
    const labelList& map,
...
void Foam::directPolyTopoChange::writeMeshStats(const polyMesh& mesh, Ostream& os)
{
...
void Foam::directPolyTopoChange::getMergeSets
(
...
void Foam::directPolyTopoChange::getFaceOrder
(
    const label nActiveFaces,
...
)
{
    if (mesh.boundaryMesh().size() != nPatches_)
    {
        FatalErrorIn
        (
            "directPolyTopoChange::compactAndReorder"
...
void Foam::directPolyTopoChange::removePoint
(
    const label pointI,
...
)
{
    if (pointI < 0 || pointI >= points_.size())
    {
        FatalErrorIn("directPolyTopoChange::removePoint(const label, const label)")
            << "illegal point label " << pointI << endl
            << "Valid point labels are 0 .. " << points_.size()-1
            << abort(FatalError);
    }
...
void Foam::directPolyTopoChange::removeFace(const label faceI, const label mergeFaceI)
{
    if (faceI < 0 || faceI >= faces_.size())
    {
        FatalErrorIn("directPolyTopoChange::removeFace(const label, const label)")
...
void Foam::directPolyTopoChange::removeCell(const label cellI, const label mergeCellI)
{
    if (cellI < 0 || cellI >= cellMap_.size())
    {
        FatalErrorIn("directPolyTopoChange::removeCell(const label, const label)")
...
)
{
    if (debug)
    {
        Pout<< "directPolyTopoChange::changeMesh(polyMesh&, const bool, const bool)"
            << endl;
    }
...
)
{
    if (debug)
    {
        Pout<< "directPolyTopoChange::changeMesh"
